                ...
                <div class="modal fade" id="forgetPassword" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            ...
                            <form id="forgetPasswordForm" action="{{ url('forgetPassword') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="modal-body pb-0">
                                    <div class="row">
                                        <div class="col mb-3">
                                            <label class="form-label" for="modalEmail">Email</label>
                                            <input type="text" id="modalEmail" name="email" required class="form-control modalEmail" tabindex="-1" />
                                        </div>
                                        <p id="forgetPassword-message"></p>
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="submit" class="btn btn-dark">Send password reset code</button>
                                </div>
                            </form>

                            <form id="verificationForm" hidden action="{{ url('verificationCode') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="modal-body pb-0">
                                    <div class="row">
                                        ...
                                        <div class="col mb-3">
                                            <label class="form-label" for="modalVerification">Verification Code</label>
                                            <input type="text" id="modalVerification" name="code" max="6" required class="form-control modalCode" placeholder="Code" tabindex="-1" />
                                        </div>
                            ...
                            <form id="resetPasswordForm" hidden action="{{ url('resetPassword') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="modal-body">
                                    <div class="row">
                                        <div class="col mb-3" hidden>
                                            <label class="form-label" for="modalEmail">Email</label>
                                            <input type="text" id="modalEmail" required name="email" class="form-control modalEmail" tabindex="-1" />
                                        </div>
                                        <div class="col mb-3" hidden>
                                            <label class="form-label" for="modalVerification">Verification Code</label>
                                            <input type="text" id="modalVerification" required name="code" class="form-control modalCode" tabindex="-1" />
                                        </div>
                                        <div class="col mb-3">
                                            <label class="form-label" for="modalPassword">New Password</label>
                                            <input type="password" id="modalPassword" name="password" required class="form-control" tabindex="-1"/>
                                        </div>
                                    </div>
                                </div>
                                <div class="modal-footer">
            ...
            document.getElementById('forgetPasswordForm').addEventListener('submit', function (e) {
                ...
                xhr.onload = function () {
                    if (xhr.status === 200) {
                        ...
                        var verificationMessage = document.getElementById('forgetPassword-message');
                        verificationMessage.innerHTML = response.message;
                        if(response.status == true){
                            // verificationMessage.classList = 'text-primary';
                            document.querySelector('#verificationForm .modalEmail').value = document.querySelector('#forgetPasswordForm .modalEmail').value;
                            document.querySelector('#forgetPassword #verificationForm').removeAttribute('hidden');
                            document.querySelector('#forgetPassword #forgetPasswordForm').setAttribute('hidden', 'true');
                        }
                        else {
                            verificationMessage.classList = 'text-danger';
                        }
            ...
            document.getElementById('verificationForm').addEventListener('submit', function (e) {
                ...
                xhr.onload = function () {
                    if (xhr.status === 200) {
                        ...
                        if(response.status == true){
                            // verificationMessage.classList = 'text-primary';
                            document.querySelector('#resetPasswordForm .modalEmail').value = document.querySelector('#verificationForm .modalEmail').value;
                            document.querySelector('#resetPasswordForm .modalCode').value = document.querySelector('#verificationForm .modalCode').value;
                            document.querySelector('#forgetPassword #resetPasswordForm').removeAttribute('hidden');
                            document.querySelector('#forgetPassword #verificationForm').setAttribute('hidden', 'true');
                        }
                        else {
                            verificationMessage.classList = 'text-danger';
                        }
