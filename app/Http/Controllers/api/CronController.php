...
class CronController extends Controller
{
    ...
    {   
        ...
        $postData = PublishPost::with(['postImages', 'postVideos'])->find($post->id);
        
        $imgPaths = $postData->postImages;
        $videoPath = $postData->postVideos;
        ...
        if ($imgPaths->isNotEmpty())
        {
            foreach ($imgPaths as $img) {
                $rm_urlPath = parse_url($img->image, PHP_URL_PATH);
                $imgPath = Str::replace('public\/storage/','storage\app/public/',public_path($rm_urlPath));
    ...
    public function youtubePublish($post)
    {
        ...
        $postData = PublishPost::with(['postVideos'])->find($post->id);
        $postVideos = $postData->postVideos;

        $videoPath = '';
        foreach($postVideos as $post){
        ...
        $tags = !empty($post->youtube_tags) ? explode(',', $post->youtube_tags) : ['tag1', 'tag2'];
        $category_id = !empty($post->youtube_category) ? $post->youtube_category : '22';


        if($client->getAccessToken()) {
            $snippet = new Google_Service_YouTube_VideoSnippet();
            $snippet->setTitle($post->post_title);
